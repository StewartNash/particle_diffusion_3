...
%File: particle_diff_3.m (MATLAB)
%Version: 0.0
%Author: Stewart Nash
%Date: February 25, 2015
%Description: 2-Dimensional simulation of non-interacting particles originating in region with constant velocity.
...
aviobj = avifile('example_03.avi', 'compression', 'None');
main_figure = figure;
...
%Initialize vairables to random values.
...
domain_2 = dilation(domain, GRID_SIZE);
domain_2 = dilation(domain_2, GRID_SIZE);
...
for i = 1 : TIME_STEPS
	particleposition = updateposition(particlevelocity, particleposition, TIME_INCREMENT, particlenumber);
    domain = updatedomain(particleposition, particlenumber, domainsize);
    domain_2 = dilation(domain, GRID_SIZE);
    domain_2 = dilation(domain_2, GRID_SIZE);
	imshow(domain_2)
